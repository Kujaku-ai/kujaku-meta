...
- **Layer 2b Paper Kev** — `kujaku-bot-kalshi15min-btc` live at `kalshi15min-btc.kujaku.ai`, tagged `v1.7.41` (v1.7.42-v1.7.46 docs/cleanup sprint in progress — see BOT.md). Strategy lab; thesis-first architecture (v1.5), risk-aware entry framework (v1.5.2), v1.6.x cleanup + analysis rebuild, v1.7.x sizing-overhaul (half-Kelly, expire-without-fill, realized-stats subsystem, SE-gated noise band, edge clamp), v1.7.8/v1.7.9 dashboard observability + Claude Communication redesign, v1.7.10–v1.7.16 PAR/scale + threshold tuning, v1.7.17 live-orderbook-at-fill, v1.7.18 hard-rules promote + playbook surgery, v1.7.19 PAR Phase-3 panel repair, v1.7.20 cumulation cleanup, v1.7.22 5d-hard schema-compliance shape fix, v1.7.24 `claude_client` resilience, v1.7.29 hedge engine introduction, v1.7.30 hedge dashboard, v1.7.31 hedge visibility across panels, v1.7.32 hedge-gate observability, v1.7.33 Anthropic 5xx retry, v1.7.34 min-TTE-at-decision 300s→60s, v1.7.35–v1.7.37 hedge-gates production unblock + threshold raise 15¢→20¢, v1.7.38 docs sync of that threshold, v1.7.39 5d-hard removal + 6d-hard demote + drift mini-call JSON fix, v1.7.41 rolling-stats window count→time + (thesis × side) aggregation + payout_math side-correctness (v1.7.40 reserved for audit doc, no code ship).
...
**Current phase:**
- Paper Kev continuing as the strategy research lab. v1.7.41 shipped 2026-05-14 (rolling-stats 14-day time window + `(thesis × side)` aggregation + payout_math side-correctness; see `AUDIT_1740_HEATMAP.md` for the motivating diagnostic and `AUDIT_1742_DASHBOARD.md` for the dashboard-side-awareness inventory that scopes v1.7.42). A v1.7.42-v1.7.46 docs/cleanup sprint is in progress (BOT.md / SYSTEM.md catch-up, dead-code removal of the legacy Jinja dashboard template + ~14 superseded `dashboard_data._build_*` functions, the `TierThesisWR` alias drop, the dashboard 3-SQL side-awareness pass); A+C re-audit of v1.7.41 deferred to ~2026-05-19 once a 30-trade settled-decision sample exists.
...
**2026-05-13 / 2026-05-14 — Hedge-gate production unblock sequence + v1.7.41 rolling-stats restructure + dashboard side-awareness audit.**
- **Hedge-gate sprint (v1.7.33–v1.7.38).** The v1.7.29-shipped hedge engine had been firing gates but never executing in production. Five-version sequence to unblock: v1.7.33 retried Anthropic 5xx errors so two recently-lost windows weren't repeated; v1.7.34 dropped the min-time-to-expiry-at-decision floor 300s→60s so R2 decisions no longer self-aborted on Anthropic latency; v1.7.35 redesigned Gate 2 from "exactly one filled position" (structurally impossible in the current R1+R2-agree-and-fill regime) to "all filled positions agree on side" with combined-profit sizing; v1.7.36 fixed a Gate 3 schema-path bug (`_extract_primary_side` was reading the wrong JSON path and always returning `None`, silently blocking every Gate 3 evaluation since v1.7.29); v1.7.37 then unblocked three more conditions — Gate 2 excludes hypothesis trades, Gate 3 returns silently on None (avoiding a dedup-cement-stale-fail bug), Gate 4 ask-cents ceiling 15¢→20¢; v1.7.38 synced the 15¢→20¢ language into the system prompt + module docstring + spec.
- **v1.7.39 rule changes.** The 2026-05-13 audit found `break_above × very_expensive` was historically profitable (+$1.48 / 67 trades, 84% WR) — the aggregate "-4.6% edge" stat had been hiding heterogeneity. Rule 5d-hard removed from the soft validator; drift `accept`/`pivot` very_expensive re-trip guards removed alongside. Rule 6d-hard demoted from `ValidationError` raise to soft warn (40% of lifetime ValidationErrors were dissent thesis-entry mismatches; the trader prompt still forbids the bad pattern). Drift mini-call ships with a dedicated JSON-only system prompt + `{"action":` assistant prefill so 15/15 recent prose-response failures stop.
- **AUDIT_1740_HEATMAP.md (v1.7.40, no code ship).** Asked "Do the heatmaps inform Claude, and do they split YES vs NO within each thesis type?" Three findings: (a) the dashboard heatmaps are dashboard-only, Claude never sees them; (b) every WR aggregation in `rolling_stats.py` and `payout_math.py` keyed on `thesis` alone, never on `(thesis, side)`; (c) the PAYOUT MATH block's "YES (tier, thesis): WR%" / "NO (tier, thesis): WR%" lines **looked** like a side × thesis cross-tab but the underlying dict was keyed `(tier, thesis)` only — the YES/NO labels were cosmetic, returning byte-identical WR for both sides at the same tier. Live SQL against production (audit §D) showed the pre-v1.7.41 rolling-100-trade window was **99% continuation / 1% reversal** — statistical noise for any side or thesis split.
- **v1.7.41 (2026-05-14) — the audit response.** Rolling-stats window switched from count to time (rolling-100 → 14-day, `ROLLING_STATS_WINDOW_DAYS=14`). New `(thesis × side)` aggregator + `THESIS_SIDE_MIN_N=10` threshold + `By thesis × side (last 14 days)` block in the rendered historical-performance prompt section (all 4 cells always emitted; sub-threshold cells render as `(n=X, accumulating; need n≥10)` rather than disappearing). `payout_math.py` keyed by the full `(tier, thesis, side)` triple — the YES/NO labels now reflect real side-conditioned WR. The dead `_build_stats_block` helper deleted from `scheduler.py` (never wired into the prompt template since v1.3). 1,543 tests pass, 1 skipped. Tech debt introduced and tracked: `TierThesisWR` alias to be dropped within 1-2 releases; `_DEFAULT_MIN_TOTAL_PER_CELL=1` parameter-default change documented in the `get_tier_thesis_wr` docstring.
- **AUDIT_1742_DASHBOARD.md (read-only inventory).** 12 WR / heatmap surfaces on the live dashboard. **Zero** already side-aware. Three SQL queries hold all twelve: `_get_overview_data` (Cont/Rev WR chips), `_get_charts_data` corpus (6 chart surfaces), and `db.get_hedge_panel_stats` (4 hedge surfaces). Adding `t.side` to the charts-corpus SELECT unblocks 6 surfaces in one upstream edit. Also flagged: the legacy Jinja template `app/templates/dashboard.html` + ~14 v1.5 `dashboard_data._build_*` and `render_*` functions are dead — superseded by `dashboard_render.render_full_dashboard`. ~600+ lines of orphan code available for cleanup. This audit scopes v1.7.42.
- **AUDIT_1743_CLEAN.md (post-v1.7.41 cleanup check).** Sections A (Claude usage of the new block, EXPLICIT/IMPLICIT/NO EVIDENCE on 30 settled decisions) and C (5-step logic gap analysis) deferred to ~2026-05-19 — v1.7.41 had only ~1 hour live at audit time, n=4 settled, vs n=30 spec. Section B confirmed multi-timeframe synthesis is happening on every decision (4/4 MIXED conflicts explicitly resolved by naming the dominant TF; 7/7 decisions cite ≥4 timeframes). Section D walked the rendered prompt — 3 redundancy items (defensible), 0 hard conflicts, 1 soft tier-vs-pooled WR confusion candidate, 0 stale labels remaining (commit `53f21a8` caught the one "rolling 100" stragler in the entry-strategy header). Section E confirmed no drift in the cells vs the v1.7.40-D baseline 1h before. Section F queued 9 doc edits (BOT.md + SYSTEM.md catch-up — this ship). Section G: 1 CRITICAL tracking item, 3 MEDIUM cleanup items, 5 DEFERRED. Section H confirmed the system prompt does **not** instruct the 5-step pattern-matching logic — Step 4 (compare current vectors → historical per-cell winners) is the predicted bottleneck, to be revisited after the A/C re-audit.
- **v1.7.42 (this ship, docs-only).** BOT.md Strategy Versions section catch-up from v1.7.34 to v1.7.41 (7 entries; v1.7.40 flagged as audit-doc-only). SYSTEM.md Layer-2b tagline updated `v1.7.20` → `v1.7.41`; "Current phase" bullet rewritten to reflect the v1.7.42-v1.7.46 cleanup sprint; this Session Log entry added. No code, no Railway deploy, no kill switch, no tag (operator's discretion).
- **Sprint going forward.** ~5 small cleanup ships (v1.7.42 docs through v1.7.46 dashboard side-awareness pass), then the deferred A+C re-audit ~2026-05-19 once v1.7.41 has 30+ settled trades.
